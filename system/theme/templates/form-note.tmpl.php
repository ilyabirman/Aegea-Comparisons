...
<div class="form-control">
  ...
  <div class="form-subcontrol">
    <div class="label input-label"><label><?= _S ('ff--title') ?></label></div>
    <input type="text"
  ...
  <div class="form-subcontrol">
    ...
    <p class="small baseline">
      ...
      <span id="livesave-button" class="keyboard-shortcut clickable-keyboard-shortcut" style="display: none"><?= _SHORTCUT ('livesave')? _SHORTCUT ('livesave') : _S ('ff--save') ?></span>
    ...
    <p class="e2-upload-error wrong" id="e2-upload-error-images-only" style="clear: left; display: none"><?= _S ('er--images-only-supported') ?></p>
    <p class="e2-upload-error wrong" id="e2-upload-error-cannot-create-thumbnail" style="clear: left; display: none"><?= _S ('er--cannot-create-thumbnail') ?></p>
    <p class="e2-upload-error wrong" id="e2-upload-error-cannot-upload" style="clear: left; display: none"><?= _S ('er--cannot-upload') ?></p>
...
<div class="form-control">
  <div class="label input-label"><label><?= _S ('ff--tags') ?></label></div>
  <style>
  .chzn-container ul { padding: 0; margin: 0; text-indent: 0 }
  .chzn-container li { padding: 0; margin: 0; text-indent: 0 }
  .chzn-container li:before { content: ''; margin: 0; padding: 0; text-indent: 0 }
  .chzn-container .no-results { display: none !important }
  </style>
  <select id="tags" name="tags[]" tabindex="3" class="width-4 chzn-select" multiple="multiple" data-placeholder=" " size="2">
    <? foreach ($content['form-note']['tags-info'] as $tag) { ?>
...
<? if (@$content['form-note']['time'] or @$content['form-note']['alias']) { ?>

<div class="form-control">
  <div class="label input-label"><label><? if (@$content['form-note']['note']['published?']) { ?><? } else { ?><? } ?></label></div>

  <div class="e2-note-time-and-url">
    <a href="javascript: return false" onclick="$ ('.e2-note-time-and-url').slideToggle(333); return false" class="dashed admin"><? if (@$content['form-note']['note']['published?']) { ?><?= _S ('ff--is-at-address') ?><? } else { ?><?= _S ('ff--will-get-address') ?><? } ?> <? if (@$content['form-note']['alias']) { ?>
    .../<?= @$content['form-note']['alias'] ?>/
    <? } ?>
    <? if (@$content['form-note']['time']) { ?>
